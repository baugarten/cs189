function [ node ] = build_node_recursive( data_labels, impurity_method, k )
...
if all(labels == labels(1))
    ...
    [feature, split] = find_best_split(data_labels, impurity_method, k);
    ...
    if index == size(data_labels, 1)
        ...
        lte_node = build_node_recursive(lte_data_labels, impurity_method, k);
        gt_data_labels = data_labels(index+1:end, :);
        gt_node = build_node_recursive(gt_data_labels, impurity_method, k);
